...
const seed={
  ...
  granny:{name:"Autumn Blanket",cols:5,rows:6,base:"#fff3e7",palette:["#d66f42","#8c5134","#f3c764","#9a7c50","#f4eee2"],squares:[]},
  currentPattern:null,
...
function renderHomeWips(){
  ...
  $("#homeWips").innerHTML=list.length?list.map(p=>`<button class="mini-project text-button" data-home-project="${p.id}"><strong>${esc(p.name)}</strong><small>${clamp(+p.progress||0,0,100)}% complete</small></button>`).join(""):`<div class="empty">No WIPs yet. Start something cute.</div>`;
...
function openProjectModal(existing=null){
  ...
  modal({title:existing?"Edit project":"New project",eyebrow:"PROJECT",fields:
    ...
    onSave:o=>{
      const obj={...p,id:p.id||id(),name:o.name,status:o.status,type:o.type,progress:+o.progress||0,hook:o.hook,yarn:o.yarn,pattern:o.pattern,startDate:o.startDate,deadline:o.deadline,notes:o.notes};
...
function openStashModal(existing=null){
  ...
  modal({title:existing?"Edit supply":"Add supply",eyebrow:"STASH",fields:
    ...
    onSave:o=>{
      const obj={...s,id:s.id||id(),category:o.category,name:o.name,colorName:o.colorName,color:o.color,weight:o.weight,qty:+o.qty||0,yardage:o.yardage,fiber:o.fiber,dyeLot:o.dyeLot,location:o.location,notes:o.notes};
...
function ensureGranny(){
  const g=data.granny;if(!g.palette?.length)g.palette=["#d66f42","#8c5134","#f3c764","#9a7c50","#f4eee2"];
...
function renderGranny(){
  ...
  $("#grannyGrid").innerHTML=g.squares.map((s,i)=>`<button class="granny-square ${s.motif?"has-motif":""}" data-square="${i}" data-status="${s.status}" style="background:${s.color};--square-border:${g.base}" title="Square ${i+1}${s.motif?" • motif":""}">${s.motif?`<img class="square-motif" src="${s.motif}" alt="">`:""}</button>`).join("");
  ...
  $("#motifSquareNumber").max=g.squares.length;
  if(+$("#motifSquareNumber").value>g.squares.length)$("#motifSquareNumber").value=g.squares.length;
  $$("[data-granny-color]").forEach(inp=>inp.onchange=()=>{g.palette[+inp.dataset.grannyColor]=inp.value;persist()});
  $$("[data-square]").forEach(b=>{
    b.onclick=e=>{
      const s=g.squares[+b.dataset.square];
      if(e.shiftKey){cycleSquareColor(s,g.palette);persist();return}
      const states=["todo","doing","done","joined"];s.status=states[(states.indexOf(s.status)+1)%states.length];persist();
    };
    b.oncontextmenu=e=>{e.preventDefault();cycleSquareColor(g.squares[+b.dataset.square],g.palette);persist()};
  });
}
...
$("#buildGranny").onclick=()=>{data.granny.name=$("#grannyName").value;data.granny.cols=clamp(+$(
"#grannyCols").value||5,1,20);data.granny.rows=clamp(+$("#grannyRows").value||6,1,20);data.granny.base=$("#grannyBase").value;ensureGranny();persist();toast("Grid updated")};
...
async function imageFileToSquareData(file,size=280){
  ...
  try{return canvas.toDataURL("image/webp",.82)}
  catch{return canvas.toDataURL("image/png")}
}
$("#applyMotif").onclick=async()=>{
  ensureGranny();
  const n=+$("#motifSquareNumber").value;
  const file=$("#motifImage").files[0];
  if(!Number.isInteger(n)||n<1||n>data.granny.squares.length)return alert(`Choose a square number from 1 to ${data.granny.squares.length}.`);
  if(!file)return alert("Choose a motif image first.");
  try{
    data.granny.squares[n-1].motif=await imageFileToSquareData(file);
    persist();
    $("#motifImage").value="";
    toast(`Motif added to square ${n}`);
  }catch(e){
    console.error(e);
...
$("#removeMotif").onclick=()=>{
  ...
  const n=+$("#motifSquareNumber").value;
  if(!Number.isInteger(n)||n<1||n>data.granny.squares.length)return alert(`Choose a square number from 1 to ${data.granny.squares.length}.`);
  delete data.granny.squares[n-1].motif;
  persist();
  toast(`Motif removed from square ${n}`);
};
...
$("#extractColors").onclick=async()=>{
  ...
  data.granny.palette=cols;persist();toast("Palette extracted from image");
};
...
function renderGallery(){
  ...
  $("#galleryGrid").innerHTML=list.length?list.map(p=>`<article class="gallery-card"><span class="eyebrow">${esc(p.type||"FINISHED")}</span><h3>${esc(p.name)}</h3><div class="meta">${p.yarn?esc(p.yarn):"Crochet victory ✨"}</div></article>`).join(""):`<div class="card empty">Finish a project and it will appear here automatically. 💕</div>`;
}
...
$("#syncPush").onclick=()=>syncRequest("push");$("#syncPush").onclick=()=>syncRequest("push");
